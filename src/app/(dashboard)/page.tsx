...
const allStatuses: StockStatus[] = ["Available", "EOI", "Under Contract", "Exchanged", "Settled"];
...
const stockData: {
  ...
  { lot: "203", bed: 3, bath: 2, car: 2, m2Int: 110, m2Ext: 18, price: "$625,000", status: "EOI", agent: "Sarah M.", updated: "1 day ago" },
...
export default function DashboardPage() {
  ...
  const filteredStock = statusFilter === "All"
    ? stockData
    : stockData.filter((row) => row.status === statusFilter);
      ...
      <Card padding="sm">
        <div className="px-4 py-4 sm:px-6 border-b border-border flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-secondary mt-0.5">{statusFilter === "All" ? "All lots" : statusFilter} for {selectedProject}</p>
          </div>
          <div className="relative">
            <select
              value={statusFilter}
